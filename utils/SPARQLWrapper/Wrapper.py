...
class SPARQLWrapper(object):
    ...
    def _query(self):
        ...
        try:
            ...
            if e.code == 400:
                raise QueryBadFormed(e.read().decode('utf-8', 'replace'))
            elif e.code == 404:
                raise EndPointNotFound(e.read().decode('utf-8', 'replace'))
            elif e.code == 401:
                raise Unauthorized(e.read().decode('utf-8', 'replace'))
            elif e.code == 414:
                raise URITooLong(e.read().decode('utf-8', 'replace'))
            elif e.code == 500:
                raise EndPointInternalError(e.read().decode('utf-8', 'replace'))
            else:
                raise e
